    ...
    def training_step(self, batch):
        if self.options['loss'] == 'ssdu':
            ...
            if self.options['exp_loss']:
                ...
                w = torch.exp(torch.Tensor([-tau*(self.options['num_stages']-k+1) for k in range(self.options['num_stages'])]).cuda())
                w /= torch.sum(w)
        ...
        # tensorboard logging
        self.log_dict({"train_loss_epoch": loss, "step": self.current_epoch*1.}, on_step=False, on_epoch=True)
